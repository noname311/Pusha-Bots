  ...
  run: async (client, message, args, embed) => {
    ...
    msg.awaitReactions((reaction, user) => ["green", "red"].includes(reaction.emoji.name) && user.id === member.user.id, {
      ...
      if (reaction.emoji.name === 'green') {
        ...
        msg.edit(`${member.toString()} üyesine başarıyla tag aldırıldı! ${green}`).then(x => x.delete({timeout: 5000}))
        await taggeds.findOneAndUpdate({ guildID: message.guild.id, userID: message.author.id }, { $push: { taggeds: member.user.id } }, { upsert: true });
      } else {
        msg.edit(`${member.toString()} üyesi, tag aldırma teklifini reddetti! ${red}`).then(x => x.delete({timeout: 5000}))
      }
    }).catch(() => msg.edit(`${red} Tag aldırma işlemi iptal edildi!`)).then(x => x.delete({timeout: 5000}));
  }
}